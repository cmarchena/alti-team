...
class PostgresUserRepository {
    async findById(id) {
        try {
            ...
            if (result.rows.length === 0) return (0, result_1.success)(null);
            return (0, result_1.success)(toCamelCase(result.rows[0]));
        }
        catch (error) {
            return (0, result_1.failure)(error instanceof Error ? error : new Error('Unknown error'));
...
class PostgresTeamMemberRepository {
    async findById(id) {
        try {
            const result = await pool.query(`SELECT tm.*, u.id as user_id, u.name as user_name, u.email as user_email 
                FROM team_members tm 
                LEFT JOIN users u ON tm.user_id = u.id 
                WHERE tm.id = $1`, [id]);
            if (result.rows.length === 0) return (0, result_1.success)(null);
            const row = result.rows[0];
            // Transform to camelCase and add nested user object
            const tm = toCamelCase(row);
            // Remove the camelCased user fields and create nested user object
            const { userId: _userId, userName: _userName, userEmail: _userEmail, ...tmData } = tm;
...
class PostgresCommentRepository {
    ...
    async findByTaskId(taskId) {
        try {
            const result = await pool.query(`SELECT c.*, u.id as author_id, u.name as author_name, u.email as author_email 
                FROM comments c 
                LEFT JOIN users u ON c.user_id = u.id 
                WHERE c.task_id = $1 ORDER BY c.created_at DESC`, [taskId]);
            const comments = result.rows.map(row => {
                const comment = toCamelCase(row);
                // Extract author fields and create nested author object
                const { authorId: _authorId, authorName: _authorName, authorEmail: _authorEmail, ...commentData } = comment;
                return {
                    ...commentData,
                    author: {
                        id: row.author_id || '',
            ...
            return (0, result_1.success)(comments);
        }
        catch (error) {
            return (0, result_1.failure)(error instanceof Error ? error : new Error('Unknown error'));
        }
