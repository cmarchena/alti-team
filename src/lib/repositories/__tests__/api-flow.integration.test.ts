...
describe('Full API Flow Integration Test', () => {
  ...
  describe('Step 1: User Registration', () => {
    it('should create a new user', async () => {
      ...
      const user = result as Success<{ id: string; email: string; name: string; password?: string; createdAt: Date; updatedAt: Date }>
  ...
  describe('Step 2: User Login', () => {
    ...
    it('should login successfully with correct credentials', async () => {
      ...
      const userResult = findResult as Success<{ id: string; email: string; name: string; password?: string }>
    ...
    it('should fail login with incorrect password', async () => {
      ...
      const userResult = findResult as Success<{ password?: string }>
      const isValid = await bcrypt.compare('wrongpassword', userResult.data.password!)
  ...
  describe('Step 3: Create Organization', () => {
    ...
    it('should create an organization owned by the user', async () => {
      ...
      expect(isSuccess(orgResult)).toBe(true)
      const org = orgResult as Success<{ id: string; name: string; ownerId: string }>
    ...
    it('should find organization by owner ID', async () => {
      ...
      const orgs = findResult as Success<Array<{ id: string; ownerId: string }>>
    ...
    it('should fail to find non-existent organization', async () => {
      ...
      expect(isSuccess(findResult)).toBe(true)
      expect((findResult as Success<null>).data).toBeNull()
    })
  })
  ...
  describe('Step 4: Create Project', () => {
    ...
    it('should create a project for the organization', async () => {
      ...
      expect(isSuccess(projectResult)).toBe(true)
      const project = projectResult as Success<{ id: string; name: string; organizationId: string; status: string }>
    ...
    it('should find projects by organization ID', async () => {
      ...
      const projects = findResult as Success<Array<{ id: string }>>
    ...
    it('should verify organization ownership before creating project', async () => {
      ...
      if (isFailure(otherUserResult)) return
      const otherUserId = (otherUserResult as Success<{ id: string }>).data.id
      ...
      if (isFailure(otherOrgResult)) return
      const otherOrgId = (otherOrgResult as Success<{ id: string }>).data.id
      ...
      expect(isSuccess(ownProjectResult)).toBe(true)

      // Verify: user cannot access other user's organization
      const orgFindResult = await repos.organizations.findById(otherOrgId)
      expect(isSuccess(orgFindResult)).toBe(true)
      const orgFindData = orgFindResult as Success<{ ownerId: string } | null>
  ...
  describe('Step 5: Create Task', () => {
    ...
    it('should create a task for the project', async () => {
      ...
      const task = taskResult as Success<{ id: string; title: string; projectId: string; priority: string }>
    ...
    it('should find tasks by project ID', async () => {
      ...
      const tasks = findResult as Success<Array<{ id: string }>>
    ...
    it('should verify project organization ownership before creating task', async () => {
      ...
      if (isFailure(otherProjectResult)) return
      const otherProjId = (otherProjectResult as Success<{ id: string }>).data.id
      ...
      expect(isSuccess(ownTaskResult)).toBe(true)

      // Verify: user cannot access other user's project
      const projectFindResult = await repos.projects.findById(otherProjId)
      expect(isSuccess(projectFindResult)).toBe(true)
      const projectFindData = projectFindResult as Success<{ organizationId: string } | null>
  ...
  describe('Full Flow: Register -> Login -> Org -> Project -> Task', () => {
    it('should complete the entire flow successfully', async () => {
      ...
      const registerData = registerResult as Success<{ id: string }>
      testUser.id = registerData.data.id

      // Step 2: Login
      const loginResult = await repos.users.findByEmail(testUser.email)
      expect(isSuccess(loginResult)).toBe(true)
      const loginData = loginResult as Success<{ password?: string }>
      const isValid = await bcrypt.compare(testUser.password, loginData.data.password!)
      expect(isValid).toBe(true)
      ...
      expect(isSuccess(orgResult)).toBe(true)
      const orgData = orgResult as Success<{ id: string }>
      ...
      expect(isSuccess(projectResult)).toBe(true)
      const projectData = projectResult as Success<{ id: string }>
      ...
      expect(isSuccess(taskResult)).toBe(true)
      const taskData = taskResult as Success<{ id: string }>
      ...
      expect(isSuccess(taskFindResult)).toBe(true)
      const taskFindData = taskFindResult as Success<{ projectId: string } | null>
