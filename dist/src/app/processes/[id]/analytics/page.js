...
exports.default = ProcessAnalyticsPage;
...
function ProcessAnalyticsPage() {
    ...
    const calculateAnalytics = (processData) => {
        try {
            const parsedSteps = JSON.parse(processData.steps || "[]");
            const processCreatedAt = new Date(processData.createdAt);
            const stepDetails = parsedSteps.map((step, index) => {
    ...
    return (<div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            ...
            <div className="flex space-x-3">
              <link_1.default href={`/processes/${process.id}`} className="inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">
                View Process
              </link_1.default>
              <link_1.default href={`/processes/${process.id}/execute`} className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700">
                Execute Process
              </link_1.default>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {analytics.stepDetails.map((step, index) => (<tr key={step.stepId}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${step.completed
                ...
                : 'bg-gray-100 text-gray-800'}`}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {step.isBottleneck && (<span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">
        ...
        {/* Bottlenecks Alert */}
        {analytics.bottlenecks.length > 0 && (<div className="bg-yellow-50 border border-yellow-200 rounded-md p-4 mb-8">
            <div className="flex">
              <div className="flex-shrink-0">
              ...
              <div className="ml-3">
                ...
                <div className="mt-2 text-sm text-yellow-700">
                  ...
                  <ul className="mt-2 list-disc list-inside">
                    {analytics.bottlenecks.map((bottleneck) => (<li key={bottleneck.stepId}>{bottleneck.name}</li>))}
                  </ul>
            ...
            <div className="ml-3">
              ...
              <div className="mt-2 text-sm text-blue-700">
                <ul className="list-disc list-inside space-y-1">
                  {analytics.completionPercentage < 100 && (<li>Complete remaining steps to improve overall process efficiency</li>)}
                  {analytics.bottlenecks.length > 0 && (<li>Review bottleneck steps for potential optimization opportunities</li>)}
                  {analytics.averageTimePerStep && analytics.averageTimePerStep > 24 && (<li>Consider breaking down long-running steps into smaller, manageable tasks</li>)}
