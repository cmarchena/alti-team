...
const createTaskTool = {
    ...
    handler: async (args, context) => {
        ...
        if (args.assigneeId)
            createData.assignedToId = args.assigneeId;
        if (args.dueDate)
            createData.dueDate = new Date(args.dueDate);
...
const updateTaskTool = {
    ...
    handler: async (args, context) => {
        ...
        if (args.assigneeId !== undefined)
            updateData.assignedToId = args.assigneeId;
        if (args.dueDate !== undefined)
            updateData.dueDate = new Date(args.dueDate);
...
const listTasksTool = {
    ...
    handler: async (args, context) => {
        ...
        const allTasksResult = await context.repositories.tasks.findByProjectId(args.projectId || 'all' // This will need to be handled properly
        );
...
const searchTasksTool = {
    ...
    handler: async (args, context) => {
        ...
        const allTasksResult = await context.repositories.tasks.findByProjectId(args.projectId || 'all');
        if (allTasksResult.isErr()) {
            return {
                content: [{ type: 'text', text: `Error: ${allTasksResult.error.message}` }],
                isError: true,
        ...
        tasks = tasks.filter(t => t.title.toLowerCase().includes(queryLower) ||
...
// Register task tools
(0, index_js_1.registerTool)(createTaskTool);
(0, index_js_1.registerTool)(getTaskTool);
(0, index_js_1.registerTool)(updateTaskTool);
(0, index_js_1.registerTool)(deleteTaskTool);
(0, index_js_1.registerTool)(listTasksTool);
(0, index_js_1.registerTool)(searchTasksTool);
(0, index_js_1.registerTool)(getMyTasksTool);
(0, index_js_1.registerTool)(addTaskCommentTool);
(0, index_js_1.registerTool)(getTaskCommentsTool);
