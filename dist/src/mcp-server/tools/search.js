        ...
        if (entityTypes.includes('project')) {
            try {
                ...
                if (membersResult.isOk()) {
                    ...
                    const filteredProjects = projectResults.filter(p => p.name.toLowerCase().includes(query) ||
                        (p.description && p.description.toLowerCase().includes(query))).slice(0, limit);
        ...
        if (entityTypes.includes('task')) {
            try {
                ...
                if (membersResult.isOk()) {
                    ...
                    const filteredTasks = taskResults.filter(t => t.title.toLowerCase().includes(query) ||
                        (t.description && t.description.toLowerCase().includes(query))).slice(0, limit);
        ...
        if (entityTypes.includes('resource')) {
            try {
                ...
                if (membersResult.isOk()) {
                    ...
                    const filteredResources = resourceResults.filter(r => r.name.toLowerCase().includes(query) ||
                        (r.metadata && JSON.stringify(r.metadata).toLowerCase().includes(query))).slice(0, limit);
...
// Register tools
(0, index_js_1.registerTool)(globalSearchTool);
