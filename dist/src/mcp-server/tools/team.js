...
const getTeamTool = {
    ...
    handler: async (args, context) => {
        ...
        if ((0, result_1.isFailure)(teamResult)) {
            return {
        ...
        const hasAccess = await (0, auth_js_1.validateOrganizationAccess)(context.userId, team.organizationId, context);
        if (!hasAccess) {
            return {
                content: [
                    {
...
const listTeamsTool = {
    ...
    handler: async (args, context) => {
        ...
        const result = await context.repositories.teams.findByOrganizationId(args.organizationId);
        if ((0, result_1.isFailure)(result)) {
            return {
                content: [{ type: 'text', text: `Error: ${result.error.message}` }],
                isError: true,
        ...
        return {
            content: [
                {
                    type: 'text',
                    text: JSON.stringify(result.data.map((team) => ({
                        id: team.id,
                        name: team.name,
                        description: team.description,
                        organizationId: team.organizationId,
...
const updateTeamTool = {
    ...
    handler: async (args, context) => {
        ...
        const teamResult = await context.repositories.teams.findById(args.teamId);
        if ((0, result_1.isFailure)(teamResult) || !teamResult.data) {
            return {
                content: [{ type: 'text', text: 'Team not found' }],
                isError: true,
            };
        ...
        const result = await context.repositories.teams.update(args.teamId, updateData);
        if ((0, result_1.isFailure)(result)) {
            return {
                content: [{ type: 'text', text: `Error: ${result.error.message}` }],
                isError: true,
...
const removeTeamMemberTool = {
    ...
    handler: async (args, context) => {
        ...
        if ((0, result_1.isFailure)(membersResult)) {
            return {
                content: [
...
const listTeamMembersTool = {
    ...
    handler: async (args, context) => {
        ...
        const members = membersResult.data.filter((m) => m.departmentId === args.teamId);
        const membersWithDetails = await Promise.all(members.map(async (member) => {
            ...
            const user = (0, result_1.isSuccess)(userResult) ? userResult.data : null;
            return {
                id: member.id,
                userId: member.userId,
...
// Register tools
(0, index_js_1.registerTool)(createTeamTool);
(0, index_js_1.registerTool)(getTeamTool);
(0, index_js_1.registerTool)(listTeamsTool);
(0, index_js_1.registerTool)(updateTeamTool);
(0, index_js_1.registerTool)(deleteTeamTool);
(0, index_js_1.registerTool)(addTeamMemberTool);
(0, index_js_1.registerTool)(removeTeamMemberTool);
(0, index_js_1.registerTool)(listTeamMembersTool);
