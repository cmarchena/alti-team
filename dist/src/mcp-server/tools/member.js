...
const getMemberTool = {
    ...
    handler: async (args, context) => {
        ...
        // Check if user has access to the organization
        const hasAccess = await (0, auth_js_1.validateOrganizationAccess)(context.userId, member.organizationId, context);
        if (!hasAccess) {
            return {
                content: [{ type: 'text', text: 'Access denied. You are not a member of this organization.' }],
                isError: true,
...
const cancelInvitationTool = {
    ...
    handler: async (args, context) => {
        ...
        // Check if user has access to the organization
        const hasAccess = await (0, auth_js_1.validateOrganizationAccess)(context.userId, invitation.organizationId, context);
        if (!hasAccess) {
            return {
                content: [{ type: 'text', text: 'Access denied. You are not a member of this organization.' }],
                isError: true,
...
// Register tools
(0, index_js_1.registerTool)(listOrganizationMembersTool);
(0, index_js_1.registerTool)(getMemberTool);
(0, index_js_1.registerTool)(inviteMemberTool);
(0, index_js_1.registerTool)(listPendingInvitationsTool);
(0, index_js_1.registerTool)(cancelInvitationTool);
(0, index_js_1.registerTool)(updateMemberRoleTool);
