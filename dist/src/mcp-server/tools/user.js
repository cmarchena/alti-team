...
const searchUsersTool = {
    ...
    handler: async (args, context) => {
        const result = await context.repositories.users.search(args.query, args.organizationId, args.limit);
        if (result.isErr()) {
            return {
                content: [{ type: 'text', text: `Error: ${result.error.message}` }],
                isError: true,
...
(0, index_js_1.registerTool)(getMyProfileTool);
(0, index_js_1.registerTool)(updateMyProfileTool);
(0, index_js_1.registerTool)(searchUsersTool);
