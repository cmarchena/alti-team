...
(0, index_js_1.registerTool)({
    ...
    handler: async (args, context) => {
        ...
        try {
            const hasAccess = await (0, auth_js_1.validateOrganizationAccess)(args.organizationId, context.userId, context);
            if (!hasAccess) {
                return {
                    content: [
            ...
            if (!slackToken) {
                return {
                    content: [
                        {
                            type: 'text',
                            text: 'Slack integration is not configured. Please set the SLACK_BOT_TOKEN environment variable.\n\n' +
...
(0, index_js_1.registerTool)({
    ...
    handler: async (args, context) => {
        ...
        try {
            ...
            const microsoftCalendarEnabled = process.env.MICROSOFT_CALENDAR_ENABLED === 'true';
            if (!googleCalendarEnabled && !microsoftCalendarEnabled) {
                return {
                    content: [
                        {
                            type: 'text',
                            text: 'Calendar integration is not configured. Please configure either Google Calendar or Microsoft Calendar.\n\n' +
