...
(0, index_js_1.registerTool)({
    ...
    description: 'Generate AI-powered insights about projects, tasks, and team performance. Identifies blockers, risks, and optimization opportunities.',
    inputSchema: {
        type: 'object',
        properties: {
            organizationId: {
    ...
    handler: async (args, context) => {
        ...
        try {
            ...
            const teamMembersResult = await context.repositories.teamMembers.findByOrganizationId(args.organizationId);
            ...
            for (const project of projects) {
                ...
                const blockedTasks = tasks.filter((t) => t.status === 'in-progress' && t.priority === 'urgent');
                if (overdueTasks.length > 0) {
                    insights.push({
                        ...
                        suggestion: 'Review and reassess priorities for overdue tasks. Consider extending deadlines or redistributing workload.',
                        relatedEntity: project.name,
                    });
                }
                const highPriorityActive = tasks.filter((t) => t.status === 'in-progress' &&
                ...
                if (highPriorityActive.length > 2) {
                    insights.push({
                        ...
                        suggestion: 'Consider queuing tasks or adding resources to maintain quality and meet deadlines.',
                        relatedEntity: project.name,
                    });
                }
                ...
                if (stalledTasks.length > 0) {
                    insights.push({
                        ...
                        suggestion: 'Follow up with assignees to unblock progress or identify obstacles.',
                        relatedEntity: project.name,
                    });
                }
                ...
                if (totalTasks > 5 && completionRate < 20) {
                    insights.push({
                        ...
                        suggestion: 'Consider daily standups or sprint reviews to accelerate progress.',
                        relatedEntity: project.name,
                    });
                }
            }
            const userTasksResult = await context.repositories.tasks.findByAssignedToId(context.userId);
            const userTasks = userTasksResult.isOk()
                ? userTasksResult.value || []
                : [];
            const myOverdue = userTasks.filter((t) => t.dueDate && new Date(t.dueDate) < now && t.status !== 'done');
            ...
            const myHighPriority = userTasks.filter((t) => (t.priority === 'high' || t.priority === 'urgent') &&
                t.status !== 'done');
            ...
            if (myHighPriority.length > 5) {
                insights.push({
                    ...
                    suggestion: 'Review your task list and consider requesting help or adjusting priorities.',
                });
            }
            const teamWorkload = [];
            for (const member of teamMembers) {
                const memberTasks = await context.repositories.tasks.findByAssignedToId(member.userId);
            ...
            if (overloadedMembers.length > 0) {
                insights.push({
                    ...
                    suggestion: 'Consider redistributing tasks to prevent burnout and maintain quality.',
                });
            }
            ...
            if (riskProjects.length > 0) {
                insights.push({
                    ...
                    suggestion: 'Review project status, assess blockers, and consider scope reduction or deadline extension.',
            ...
            for (const [category, categoryInsights] of Object.entries(insightsByCategory)) {
                output += `### ${categoryLabels[category] || category}\n\n`;
...
(0, index_js_1.registerTool)({
    ...
    description: 'Get a single proactive suggestion based on current context. Useful for surfacing one relevant insight at a time.',
    inputSchema: {
        type: 'object',
        properties: {
            organizationId: {
    ...
    handler: async (args, context) => {
        ...
        try {
            ...
            for (const project of projects) {
                ...
                const blockedTasks = tasks.filter((t) => t.priority === 'urgent' && t.status === 'in-progress');
