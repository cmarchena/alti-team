...
const generateProjectReportTool = {
    ...
    handler: async (args, context) => {
        ...
        if ((0, result_1.isFailure)(projectResult)) {
            return {
                content: [
        ...
        const hasAccess = await (0, auth_js_1.validateOrganizationAccess)(context.userId, project.organizationId, context);
        if (!hasAccess) {
            return {
        ...
        if (args.format === 'summary') {
            ...
            if (completionRate < 25 && project.endDate) {
                const daysUntilDeadline = Math.floor((new Date(project.endDate).getTime() - Date.now()) /
                    (1000 * 60 * 60 * 24));
            ...
            report.executiveSummary = {
                status: project.status,
                overallHealth: overdueTasks > 0 || urgentTasks > 0 ? 'at_risk' : 'on_track',
                overallProgress: completionRate.toFixed(1) + '%',
                keyMetrics: {
                    totalTasks,
                    completedTasks,
                ...
                timeline: {
                    ...
                    daysRemaining: project.endDate
                        ? Math.max(0, Math.floor((new Date(project.endDate).getTime() - Date.now()) /
                            (1000 * 60 * 60 * 24)))
                        : null,
                },
            ...
            if (overdueTasks > 0) {
                report.executiveSummary.recommendations.push(`Address ${overdueTasks} overdue task(s) to get back on schedule`);
            }
            if (urgentTasks > 0) {
                report.executiveSummary.recommendations.push(`Escalate ${urgentTasks} urgent task(s) for immediate attention`);
            }
            if (completionRate < 50 && project.endDate) {
                const daysRemaining = Math.floor((new Date(project.endDate).getTime() - Date.now()) /
                    (1000 * 60 * 60 * 24));
                if (daysRemaining > 0) {
                    report.executiveSummary.recommendations.push(`Accelerate delivery pace to meet ${daysRemaining} day deadline`);
                }
            }
            if (report.executiveSummary.recommendations.length === 0) {
                report.executiveSummary.recommendations.push('Project is progressing well');
            }
        }
        return {
...
const generateTeamReportTool = {
    ...
    handler: async (args, context) => {
        ...
        const teamMembersResult = await context.repositories.teamMembers.findByOrganizationId(context.userId);
        if ((0, result_1.isFailure)(teamMembersResult)) {
            return {
                content: [
        ...
        const teamMembers = teamMembersResult.data.filter((m) => m.id === args.teamId);
        ...
        const hasAccess = await (0, auth_js_1.validateOrganizationAccess)(context.userId, teamMember.organizationId, context);
        if (!hasAccess) {
            return {
                content: [{ type: 'text', text: 'Access denied' }],
                isError: true,
        ...
        const teamMembersListResult = await context.repositories.teamMembers.findByOrganizationId(teamMember.organizationId);
        const teamMembersList = (0, result_1.isSuccess)(teamMembersListResult)
            ? teamMembersListResult.data.filter((m) => m.organizationId === teamMember.organizationId)
            : [];
        const memberStats = await Promise.all(teamMembersList.map(async (member) => {
            const tasksResult = await context.repositories.tasks.findByAssignedToId(member.id);
            const tasks = (0, result_1.isSuccess)(tasksResult) ? tasksResult.data : [];
            const completedTasks = tasks.filter((t) => t.status === 'done').length;
            const inProgressTasks = tasks.filter((t) => t.status === 'in-progress').length;
            const overdueTasks = tasks.filter((t) => {
        ...
        const totalTasks = memberStats.reduce((sum, m) => sum + m.taskCounts.total, 0);
        const completedTasks = memberStats.reduce((sum, m) => sum + m.taskCounts.completed, 0);
        const overdueTasks = memberStats.reduce((sum, m) => sum + m.taskCounts.overdue, 0);
        ...
        if (requestedMetrics.includes('task_distribution')) {
            const statusDistribution = {
                todo: memberStats.reduce((sum, m) => sum +
                    m.taskCounts.total -
                    m.taskCounts.completed -
                    m.taskCounts.inProgress, 0),
                'in-progress': memberStats.reduce((sum, m) => sum + m.taskCounts.inProgress, 0),
                done: completedTasks,
                overdue: overdueTasks,
            };
        ...
        report.summary = {
            ...
            overallHealth: overdueTasks > teamMembersList.length * 0.3
                ? 'needs_attention'
                : 'healthy',
...
(0, index_js_1.registerTool)(generateProjectReportTool);
(0, index_js_1.registerTool)(generateTeamReportTool);
