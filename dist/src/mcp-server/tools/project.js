...
const createProjectTool = {
    ...
    handler: async (args, context) => {
        ...
        if (args.startDate)
            createData.startDate = new Date(args.startDate);
        if (args.endDate)
            createData.endDate = new Date(args.endDate);
...
const updateProjectTool = {
    ...
    handler: async (args, context) => {
        ...
        if (args.endDate !== undefined)
            updateData.endDate = new Date(args.endDate);
...
const listProjectsTool = {
    ...
    handler: async (args, context) => {
        ...
        const allProjectsResult = await context.repositories.projects.findByOrganizationId(args.organizationId || 'all' // This will need to be handled properly
        );
...
const createProjectFromTemplateTool = {
    ...
    handler: async (args, context) => {
        ...
        if (args.customizations?.startDate)
            createData.startDate = new Date(args.customizations.startDate);
        if (args.customizations?.endDate)
            createData.endDate = new Date(args.customizations.endDate);
        const result = await context.repositories.projects.create(createData);
        if (result.isErr()) {
            return {
                content: [{ type: 'text', text: `Error: ${result.error.message}` }],
...
const getProjectProgressTool = {
    ...
    handler: async (args, context) => {
        ...
        if (endDate) {
            ...
            if (timeProgress < 30)
                timeStatus = 'Early stage';
            else if (timeProgress < 70)
                timeStatus = 'Mid progress';
            else if (timeProgress < 100)
                timeStatus = 'Final stage';
            else
                timeStatus = 'Time completed';
        }
        return {
            content: [
                {
...
(0, index_js_1.registerTool)(createProjectTool);
(0, index_js_1.registerTool)(getProjectTool);
(0, index_js_1.registerTool)(updateProjectTool);
(0, index_js_1.registerTool)(deleteProjectTool);
(0, index_js_1.registerTool)(listProjectsTool);
(0, index_js_1.registerTool)(createProjectFromTemplateTool);
(0, index_js_1.registerTool)(listProjectTemplatesTool);
(0, index_js_1.registerTool)(getProjectAnalyticsTool);
(0, index_js_1.registerTool)(getProjectProgressTool);
