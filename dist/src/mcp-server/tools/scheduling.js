...
(0, index_js_1.registerTool)({
    name: 'suggest_task_schedule',
    description: 'Get AI-powered suggestions for task scheduling based on priorities, deadlines, and team capacity',
    inputSchema: {
        type: 'object',
        properties: {
            projectId: {
    ...
    handler: async (args, context) => {
        ...
        try {
            ...
            const orgAccessResult = await (0, auth_js_1.validateOrganizationAccess)(context.userId, project.organizationId, context);
            ...
            const teamMembersResult = await context.repositories.teamMembers.findByProjectId(args.projectId);
            ...
            for (const task of sortedByPriority) {
                ...
                const deadlineWarning = hasDeadline
                    ...
                    : '';
                const memberName = teamMembers.find((m) => m.userId === suggestedAssignee)?.position ||
                    'unassigned';
                const reasoning = task.priority === 'urgent'
            ...
            const totalEstimatedHours = suggestions.reduce((sum, s) => sum + s.estimatedHours, 0);
            ...
            const output = {
                ...
                summary: {
                    ...
                    recommendation: suggestions.length > teamMembers.length
            ...
            return {
                content: [
                    {
                        type: 'text',
                        text: `# Schedule Suggestions for "${output.project}"\n\n` +
                            `## Summary\n` +
...
(0, index_js_1.registerTool)({
    ...
    description: 'Optimize task distribution across team members to balance workload and meet deadlines',
    inputSchema: {
        type: 'object',
        properties: {
            teamId: {
    ...
    handler: async (args, context) => {
        ...
        try {
            ...
            const orgAccessResult = await (0, auth_js_1.validateOrganizationAccess)(context.userId, team.organizationId, context);
            if (orgAccessResult.isErr()) {
                return {
            ...
            const teamMembersResult = await context.repositories.teamMembers.findByOrganizationId(team.organizationId);
            const teamMembers = teamMembersResult.isOk()
                ? teamMembersResult.value?.filter((m) => m.role === 'member' || m.role === 'lead') || []
                : [];
            ...
            const relevantTasks = allTasks.filter((task) => {
                const assigned = task.assignedToId && teamUserIds.includes(task.assignedToId);
            ...
            for (const member of teamMembers) {
                const memberTasks = relevantTasks.filter((t) => t.assignedToId === member.userId);
            ...
            const totalHours = memberWorkloads.reduce((sum, m) => sum + m.currentHours, 0);
            const targetHours = totalHours / memberWorkloads.length || 0;
            const maxAllowed = targetHours * 1.2;
            const overloaded = memberWorkloads.filter((m) => m.currentHours > maxAllowed);
            const underloaded = memberWorkloads.filter((m) => m.currentHours < targetHours * 0.8);
            const suggestions = [];
            if (overloaded.length > 0 &&
                underloaded.length > 0 &&
                args.optimizeFor !== 'efficiency') {
                const sortedOverloaded = [...overloaded].sort((a, b) => b.currentHours - a.currentHours);
                const sortedUnderloaded = [...underloaded].sort((a, b) => a.currentHours - b.currentHours);
                for (const overloadedMember of sortedOverloaded) {
                    for (const task of overloadedMember.tasks) {
                        if (overloadedMember.currentHours - task.hours >= targetHours) {
                            const underloadedMember = sortedUnderloaded.find((m) => m.memberId !== overloadedMember.memberId);
                            if (underloadedMember) {
                                ...
                                overloadedMember.tasks = overloadedMember.tasks.filter((t) => t.id !== task.id);
                                if (overloadedMember.currentHours <= maxAllowed ||
            ...
            const bottlenecks = relevantTasks
                .filter((t) => {
                const daysUntilDue = Math.ceil((new Date(t.dueDate).getTime() - now.getTime()) /
                    (1000 * 60 * 60 * 24));
                ...
                assignee: teamMembers.find((m) => m.userId === t.assignedToId)?.position ||
                    'Unknown',
            }));
            const output = {
                team: team.name,
                period: args.period,
                workloads: memberWorkloads.map((m) => ({
                    member: m.name,
                    hours: m.currentHours,
                    tasks: m.taskCount,
                    status: m.currentHours > maxAllowed
                        ? 'Overloaded'
                ...
                optimization: {
                    ...
                    underloaded: underloaded.map((m) => m.name),
                    suggestions,
                },
                risks: {
                    bottlenecks,
            ...
            const workloadsTable = output.workloads
                .map((w) => `| ${w.member} | ${w.hours}h | ${w.tasks} tasks | ${w.status} |`)
                .join('\n');
            return {
                content: [
                    {
                        type: 'text',
                        text: `# Team Workload Analysis: "${output.team}"\n` +
                            ...
                            (suggestions.length > 0
                                ? suggestions
                                    .map((s) => `- Move "${s.taskTitle}" from ${s.fromMember} → ${s.toMember}\n  Reason: ${s.reason}`)
                                    .join('\n\n')
                            ...
                            (bottlenecks.length > 0
                                ? `### Critical Tasks\n\n` +
                                    bottlenecks
                                        .map((b) => `- "${b.title}" (Due: ${b.dueDate}, Assigned: ${b.assignee})`)
                                        .join('\n')
                                : ''),
                    },
                ],
